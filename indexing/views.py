...
def market_details_refresh(request):
    ...
    old_market_details = {
        i["symbol"]: i["market_cap_rank"]
        for i in list(Market_Details.objects.all().values("symbol", "market_cap_rank"))
    }
    index_market_data(market_details, old_market_details)
    for i in market_details.values():
        i["ath_date"] = make_aware(
            datetime.strptime(i["ath_date"][:19], "%Y-%m-%dT%H:%M:%S")
        )
        i["atl_date"] = make_aware(
            datetime.strptime(i["atl_date"][:19], "%Y-%m-%dT%H:%M:%S")
        )
        i["last_updated"] = make_aware(
            datetime.strptime(i["last_updated"][:19], "%Y-%m-%dT%H:%M:%S")
        )
        i["roi"] = 0.0
        if i.get("old_rank"):
            del i["old_rank"]
