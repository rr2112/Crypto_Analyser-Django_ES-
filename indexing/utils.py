...
def get_all_coin_pairs(*derivative):
    ...
    all_coin_pairs = [
        i for i in all_coin_pairs if len(i.split("/")) > 1 and i.split("/")[1] == "USDT"
    ]
...
def get_candle_data_with_timestamp(exchange, symbol, interval, limit, derivative):
    ...
    try:
        ...
        ts = [
            datetime.fromtimestamp(int(i / 1000)).strftime("%Y-%m-%d %H:%M:%S")
            for i in timestamps
        ]
...
def stochastic_crossover(df):
    ...
    df2 = pd.concat(
        [
            df,
            pd.Series(stch_vs_ma_100).rename("trend"),
            diff.rename("momentum"),
            slowk.rename("slowk"),
            slowd.rename("slowd"),
        ],
        axis=1,
    ...
    cross_over = [
        "crossover" if df2["momentum"][i - 1] != df2["momentum"][i] else "None"
    ...
    final_df = pd.concat(
        [
            df2,
            pd.Series(cross_over).rename("crossover"),
            pd.Series(ma_100[1]).rename("ma_100"),
        ],
        axis=1,
    )
    return final_df
