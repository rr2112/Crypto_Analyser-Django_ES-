...
from indexing.utils import (
    get_all_coin_pairs,
    get_candle_data_with_timestamp,
    get_avg_tr,
    get_basic_indicators,
    stochastic_crossover,
    update_candle_status,
...
def scalping_report(coin_pair, time_frame, derivative, exchange="binance"):
    ...
    market_data = Market_Details.objects.filter(
        symbol=(coin_pair.rstrip("/USDT").rstrip("/BUSD")).lower()
    ).values()
    ...
    try:
        ...
        df2["current_amplitude%(Hi-Op)"] = (
            (df2["high"] - df2["open"]) * 100 / df2["open"]
        )
        df2["open-close"] = (df2["close"] - df2["open"]) * 100 / df2["open"]
        ...
        df2["vol_change"] = round(df2["volume"] / df2["volume"].shift(periods=1), 2)
...
def main(all_coin_pairs, all_time_frames, derivative, exchange):
    ...
    with ThreadPoolExecutor(max_workers=no_of_workers) as executor:
        future_to_f_detail = {
            executor.submit(scalping_report, id, timeframe, derivative, exchange): (
                id,
                timeframe,
                derivative,
                exchange,
            )
            for timeframe in all_time_frames
            for id in all_coin_pairs[:100]
        }
