...
from indexing.utils import get_all_coin_pairs, get_candle_data_with_timestamp, get_avg_tr, get_basic_indicators, stochastic_crossover
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        result = main(market_details,all_coin_pairs,all_time_frames)
        print(result)



def scalping_report(coin_pair, time_frame,market_details):
    rsi_period = 5
    ...
    df2['market_cap_rank'] = market_details.get(coin_pair.rstrip('/USDT').rstrip('/BUSD'), {}).get(
        'market_cap_rank')
    df2['market_cap'] = market_details.get(coin_pair.rstrip(
        '/USDT').rstrip('/BUSD'), {}).get('market_cap')
...
@timeit
def main(market_details,all_coin_pairs,all_time_frames):
    global atr_weightage_filter
    required_coins = pd.DataFrame()
    with ThreadPoolExecutor(max_workers=50) as executor:
        future_to_f_detail = {executor.submit(scalping_report, id, timeframe,market_details): (id, timeframe,market_details) for timeframe in all_time_frames for id in
                              all_coin_pairs}
